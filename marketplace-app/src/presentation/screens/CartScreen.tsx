...
export function CartScreen({ onCheckout }: Props) {
  ...
  const { state, retry, updateQuantity, removeItem } = useCartViewModel();
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                <Pressable
                  ...
                  hitSlop={8}
                  style={{ paddingHorizontal: 10, paddingVertical: 2, borderWidth: 1, borderColor: theme.border, borderRadius: 6 }}
                >
                  <Text style={{ color: theme.text, fontSize: 16 }}>-</Text>
                </Pressable>
                <Text style={{ color: theme.textMuted, marginHorizontal: 10 }}>Qty {item.quantity}</Text>
                <Pressable
                  onPress={() => updateQuantity(item.id, item.quantity + 1)}
                  accessibilityRole="button"
